function varargout = advanced_comparison_statistics(varargin)
...
[BIAS, RMSE, RR, NS, FAC2] = statistics_two_sets(obs,model,NS_mean);
varargout{1} = BIAS;
varargout{2} = RMSE;
varargout{3} = RR;
varargout{4} = NS;
...
NMSE = mean((BIAS).^2)/mean(obs.*model);


RR = corrcoef(obs,model);

varargout{1} = [mean(BIAS), mean(NBIAS)];
varargout{2} = [RMSE, NRMSE, NRMSE2, NMSE];
varargout{3} = RR(1,2);
...
% Calculate ratio of data between .5 and 2.0
fac2 = model./obs;
ind_fac2 = find(fac2>=.5 & fac2<=2.0);

FAC2 = length(ind_fac2)/length(fac2);
varargout{5} = FAC2;
